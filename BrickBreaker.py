...
class Overlay(pg.sprite.Sprite):
    ...
    def rerender(self): #renders self.text at specified position
...
class Block(pg.sprite.Sprite):
    ...
    def life(self): #determines how much hp the block should have based on its color
    ...
    def hit(self): #Called when a ball collides with block, handles hp and color change based on losing hp
        self.hp -= 24 #24 per hit instead of 25 as per directions because 24/3 = 8
        if self.hp <= 0:
        ...
        for block in Ball.blocks: #Handle block collision
            ...
            if pg.Rect.colliderect(self.rect, block): #using built in collision detection
                ...
                if self.rect.right > block.rect.x  and self.rect.left < block.rect.x + 100:
                    ...
                    self.velocity[1] *= -1.03
                    self.velocity[0] *= -1.03
                self.velocity[0] = min(12, self.velocity[0]) #cap velocity at 12
        ...
        while self.__running:
            ...
            if blocks <= 0: #Win/lose conditions else keep displaying lives and score
                for ball in self.balls:
                    ball.kill()
                self.infoText.setText("You Win! Score: " + str(self.score))
            elif self.lives <= 0:
                for ball in self.balls:
                    ball.kill()
